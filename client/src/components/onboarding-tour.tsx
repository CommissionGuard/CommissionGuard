...
export function OnboardingTour({ isOpen, onClose, onComplete }: OnboardingTourProps) {
  ...
  const updateHighlightPosition = () => {
    ...
    if (element) {
      ...
      if (step.target === 'dashboard-header') {
      ...
      } else if (step.target === 'nav-clients' || step.target === 'nav-contracts' || step.target === 'nav-showings' || step.target === 'nav-commission-protection' || step.target === 'nav-alerts' || step.target === 'nav-records' || step.target === 'nav-intelligence' || step.target === 'nav-reports') {
        setHighlightPosition({
        ...
        setHighlightPosition({
          top: rect.top, // Use same format as other tabs
          left: rect.left - 10, // Center the box over the tab
          width: rect.width + 20, // Expand width for better centering
          height: rect.height - 30 // Reduce height to move bottom higher
        });
