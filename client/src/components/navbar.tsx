...
export default function Navbar() {
  ...
  const navItems = [
    ...
    { path: "/achievements", label: "Achievements", icon: Trophy, shortLabel: "Progress", description: "Track your milestones and earn badges" },
          ...
          <div className="max-w-7xl mx-auto px-2 sm:px-4 lg:px-6">
            <div className="flex justify-between items-center overflow-x-auto scrollbar-hide">
              {navItems.map((item, index) => {
                ...
                // Special handling for Showing Tracker tab
                if (item.path === "/showing-tracker") {
                  return (
                    <motion.button
                      key={item.path}
                      onClick={() => setLocation(item.path)}
                ...
                // Special handling for Public Records Monitor tab
                if (item.path === "/public-records") {
                  return (
                    <motion.button
                      key={item.path}
                      onClick={() => setLocation(item.path)}
